...
func GetMilestones(w http.ResponseWriter, r *http.Request) {
	query := `SELECT id, title, organization, milestone_type, description, date_label, sort_order, created_at, updated_at 
	...
	for rows.Next() {
		...
		err := rows.Scan(&m.ID, &m.Title, &m.Organization, &m.MilestoneType, &m.Description, &m.DateLabel, &m.SortOrder, &m.CreatedAt, &m.UpdatedAt)
		if err != nil {
...
func CreateMilestone(w http.ResponseWriter, r *http.Request) {
	...
	query := `INSERT INTO milestones (title, organization, milestone_type, description, date_label, sort_order)
	          VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, created_at, updated_at`

	err := admin.DB.QueryRow(query, m.Title, m.Organization, m.MilestoneType, m.Description, m.DateLabel, m.SortOrder).
...
func UpdateMilestone(w http.ResponseWriter, r *http.Request) {
	...
	query := `UPDATE milestones SET title = $1, organization = $2, milestone_type = $3, 
	          description = $4, date_label = $5, sort_order = $6, updated_at = now() WHERE id = $7`

	result, err := admin.DB.Exec(query, m.Title, m.Organization, m.MilestoneType, m.Description, m.DateLabel, m.SortOrder, id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
